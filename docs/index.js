...
window.addEventListener(
	//fires when the entire page, including all dependent resources (like images, stylesheets, etc.), has finished loading.
	'load',
	async function() {
...
async function basetrip() {
	...
	for(i=0;i<CL.length;i++) {
		...
		CHAINS[CL[i]].COFT = new ethers.Contract( CHAINS[CL[i]].oft , LPABI, CHAINS[CL[i]].pp)
		CLP.push( CHAINS[CL[i]].pp );
		ALLCOFTS.push( CHAINS[CL[i]].COFT)
	}
...
function paintStaticSettingsTableHeads(){
	$("settings-table").innerHTML = `
		<div class="c2a90-row">
			...
			<div onclick="sortit(0, 'supplies-table', 'c2a90-row', 'c2a90-row-item', null, 1, 0)">Availability				<br><span class="c2a90-row-byline">Ping Test</span></div>
...
async function dexstats() {
	...
	// SUPPLIES TABLE
	try {
		C_TVLGURU = new ethers.Contract( TOKEN_PRICE_TVLGURU,["function p_t_coin_usd(address) public view returns(uint)"], CHAINS[ TOKEN_PRICE_CHAIN ].pp);
		_price_token = (await C_TVLGURU.p_t_coin_usd( TOKEN_PRICE_LP ))/1e18;

		_supplies = (await Promise.all(ALLCOFTS.map(i=> i.totalSupply() ))).map( i=> Number(i)/1e18 )
...
async function gubs() {
	...
	// PORTFOLIO TABLE
	try {
		_userbals= (await Promise.all(ALLCOFTS.map(i=> i.balanceOf( window.ethereum.selectedAddress ) ))).map( i=> Number(i)/1e18 )
	...
	_allgascalls = _alltolzids.map(
		i=> {
			return {
				allowFailure: true,
				target: ALLCOFTS[_curnet.clindex].address ,
				callData: (
					ALLCOFTS[ _curnet.clindex ].interface.encodeFunctionData( //
						"estimateSendFee" ,
						[
							i,
...
async function bridge_gasCheck(_toclid) {
	let _gasreq = await ALLCOFTS[_curnet.clindex].estimateSendFee(
...
async function bridge_submit(_toclid) {
	...
	if( Number(_oamt/(10n**18n)) > _userbals[_curnet.clindex] ) {
		notice(`
			...
			<br><br>
			<b>Please reduce the amount and retry again, or accumulate some more ${TOKEN_NAME}.
		`);
		return;
	}
	...
	let _bridgefee = await ALLCOFTS[_curnet.clindex].quoteOFTFee( CHAINS[CL[_toclid]].lzid , _oamt );
	...
	ALLCOFTS[_curnet.clindex] = ALLCOFTS[_curnet.clindex].connect(signer);

	let _txh = await ALLCOFTS[_curnet.clindex].sendFrom(
		window.ethereum.selectedAddress ,
		CHAINS[CL[_toclid]].lzid ,
